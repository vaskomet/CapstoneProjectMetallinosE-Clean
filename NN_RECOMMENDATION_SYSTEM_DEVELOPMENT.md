...
### Phase 4: FastAPI Microservice Setup ✅ COMPLETE

**Date**: November 9, 2025  
**Duration**: 2 hours  
**Status**: Successfully integrated NN model into existing ml-service

**Objectives**:
- ✅ Extended existing FastAPI ml-service to support neural network model
- ✅ Implemented NN prediction endpoints alongside collaborative filtering
- ✅ Configured model loading with preprocessing artifacts
- ✅ Deployed via Docker with hot-reload for development

**Implementation Details**:

**1. Model Integration** (ml-service/main.py):
- Created `NNModelManager` class to handle NN model lifecycle
- Loads PyTorch model (152,833 parameters) from checkpoint
- Loads scikit-learn StandardScaler for feature normalization
- Loads preprocessing config for metadata
- Runs on CPU with evaluation mode for inference
- Model loaded on service startup (async startup event)

**2. API Endpoints Implemented**:
- `POST /predict/nn` - Single prediction (accepts 427 features)
  - Input: Pydantic model with feature validation
  - Processing: Feature scaling → NN inference
  - Output: Match score [0-1], denormalized rating [5-10], inference time
  - Validation: Checks for NaN/Inf, feature count
  
- `POST /predict/nn/batch` - Batch predictions (up to 100)
  - Optimized for multiple predictions in single request
  - Returns individual predictions with timing
  
- `GET /model/nn/info` - Model metadata
  - Returns architecture, parameters, num_features
  - Would return test metrics (R², MSE, MAE) if in config

**3. Pydantic Models**:
- `NNPredictionRequest`: 427 features (min/max validation)
- `NNBatchPredictionRequest`: List of feature arrays (max 100)
- `NNPredictionResponse`: Match score, denormalized rating, inference time
- `NNBatchPredictionResponse`: Batch results with total time
- `NNModelInfo`: Model architecture and metadata
...
│   ├── NN Pydantic models
│   ├── load_nn_model() startup event
│   └── 3 NN endpoints
├── nn_model.py (copied from backend)
│   └── CleanerMatchNN architecture
├── models/nn/
│   ├── best_model.pth (1.8MB) - Trained weights
│   ├── feature_scaler.pkl (11KB) - StandardScaler
│   └── preprocessing_config.json (7KB) - Metadata
└── requirements.txt
    └── Added joblib==1.4.2
```

**5. Docker Integration**:
- Updated ml-service container with NN model files
- Model files copied to `/app/models/nn/` in container
- Service successfully loads NN model on startup (0.34s load time)
- Both collaborative filtering and NN models run in same service
- Health endpoint confirms model loaded status

**6. Testing Results**:
- ✅ Model loaded: 152,833 parameters
- ✅ Scaler loaded: 427 features
- ✅ Endpoints responding (HTTP 200)
- ✅ Model info endpoint returns architecture details
- ✅ Container healthy status

**7. Performance Metrics**:
- Model loading time: ~0.34s (CPU)
- Single prediction: ~31ms inference time
- Batch prediction: ~50ms for 10 predictions (~5ms each)
- Memory footprint: Minimal (CPU PyTorch)

**Architectural Decisions**:

**Why Extend Existing ml-service?**
- ✅ Reuses infrastructure (Docker, FastAPI, monitoring)
- ✅ Single service for all ML models (collaborative + NN)
- ✅ Consistent API patterns across endpoints
- ✅ Shared CORS, logging, error handling
- ❌ Could have separate service, but adds complexity

**Why Not Separate NN Service?**
- Both models serve similar purpose (recommendations)
- Resource usage is low (both can share container)
- Simplified deployment and maintenance
- Can add model router later if needed

**Feature Engineering Strategy**:
- API expects **unscaled** 427 features
- Service applies StandardScaler transform
- Matches training pipeline (fit on train, transform on inference)
- Ensures consistent normalization (mean=0, std=1)

**Challenges Encountered**:

1. **Docker Volume Mounting** ⚠️
   - Issue: Model files not visible in container after local creation
   - Cause: Volume mount already existed, new files not synced
   - Solution: `docker cp` files into running container
   - Lesson: Create model directories before first container start

2. **Feature Count Mismatch** ⚠️
   - Issue: Dataset has 434 columns but model expects 427
   - Cause: 6 metadata columns excluded during preprocessing
   - Columns excluded: meta_job_id, meta_client_id, meta_cleaner_id, meta_property_id, meta_scheduled_date, meta_review_id
   - Solution: Documented in preprocessing config
   - Impact: Need to extract exact same 427 features for inference

3. **Test Data Scaling** ⚠️
   - Issue: Initial test with scaled features returned 0.0 score
   - Cause: Sent pre-scaled test data, service scaled again (double-scaling)
   - Solution: API expects **unscaled** features, service applies scaler
   - Validation: Model works correctly with proper input

**Phase 4 Deliverables**:
- ✅ ml-service/main.py with NN support (377+ lines added)
- ✅ 3 NN endpoints functional (/predict/nn, /predict/nn/batch, /model/nn/info)
- ✅ Pydantic request/response models with validation
- ✅ Model loading on startup with error handling
...
Phase 5 will need:
1. **Feature Extraction Service** (Django)
   - Replicate `extract_nn_features.py` logic for real-time inference
   - Extract 427 features from job/cleaner/property/historical data
   - Handle missing data gracefully
   - Cache computed features where possible

2. **ML Client** (Django → FastAPI)
   - HTTP client to call `/predict/nn` endpoint
   - Error handling and fallback to hybrid scoring
   - Timeout management
   - Retry logic

3. **Integration Testing**
   - End-to-end flow: Django extracts features → FastAPI prediction
   - Verify predictions match offline model results
